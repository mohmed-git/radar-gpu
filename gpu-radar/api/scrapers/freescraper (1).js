...
// يتجاوز الحجب والـ CAPTCHAs تلقائياً
// مفتاح API: يُحمَّل من SCRAPER_API_KEY في .env
const SCRAPER_API_KEY = process.env.SCRAPER_API_KEY || '1dfdd4e2ecd3ff0174b1715c8b6af0809fb5e9c1';
const SCRAPER_API_BASE = 'api.scraperapi.com';

// جلب URL عادي (للـ JSON API)
function fetchURL(hostname, path, extraHeaders = {}) {
  return new Promise((resolve) => {
    const targetUrl = encodeURIComponent(`https://${hostname}${path}`);
    const scraperPath = `/?api_key=${SCRAPER_API_KEY}&url=${targetUrl}&keep_headers=true`;

    const req = https.request(
      {
        hostname: SCRAPER_API_BASE,
        path: scraperPath,
        method: 'GET',
        headers: {
...
// جلب صفحة HTML كاملة (للـ scraping مع JS rendering)
function fetchPage(targetUrl, extraParams = '') {
  return new Promise((resolve) => {
    const encoded = encodeURIComponent(targetUrl);
    const scraperPath = `/?api_key=${SCRAPER_API_KEY}&url=${encoded}&render=true${extraParams}`;
...
async function fetchBestBuy(product) {
  if (!product.bestbuy_sku) return null;
  try {
    const url = `https://www.bestbuy.com/site/${product.bestbuy_sku}.p?skuId=${product.bestbuy_sku}`;
    const { status, body } = await fetchPage(url, '&country_code=us');
    if (status !== 200 || !body) return null;

    // BestBuy embeds price in multiple ways
    // Method 1: JSON-LD
    const ldMatch = body.match(/"price"\s*:\s*"?([\d]+\.?\d*)"?/);
    if (ldMatch) {
      const price = parseFloat(ldMatch[1]);
      if (price > 50 && price < product.basePrice * 2) {
        console.log(`[BestBuy] ✓ ${product.name}: $${price}`);
        return price;
      }
    }

    // Method 2: priceBlock
    const priceMatch = body.match(/class="[^"]*priceView[^"]*"[^>]*>\s*\$\s*([\d,]+\.?\d*)/);
    if (priceMatch) {
      const price = parseFloat(priceMatch[1].replace(',', ''));
      if (price > 50 && price < product.basePrice * 2) {
        console.log(`[BestBuy HTML] ✓ ${product.name}: $${price}`);
        return price;
      }
    }
  } catch (err) {
    console.warn(`[BestBuy] Error for ${product.name}:`, err.message);
  }
  return null;
}

// ── AMAZON SCRAPER ────────────────────────────────────────
async function fetchAmazon(product) {
  if (!product.asin) return null;
  try {
    const url = `https://www.amazon.com/dp/${product.asin}`;
    // ScraperAPI مع country_code=us للحصول على أسعار أمريكية
    const { status, body } = await fetchPage(url, '&country_code=us&device_type=desktop');
    if (status !== 200 || !body) return null;
    ...
    if (ldMatch) {
      const price = parseFloat(ldMatch[1]);
      if (price > 50 && price < product.basePrice * 2.5) {
        console.log(`[Amazon JSON-LD] ✓ ${product.name}: $${price}`);
        return price;
      }
    }

    // Method 2: a-price-whole + a-price-fraction
    const wholeMatch = body.match(/class="a-price-whole">(\d+)</);
    const fracMatch  = body.match(/class="a-price-fraction">(\d+)</);
    if (wholeMatch) {
      const price = parseFloat(`${wholeMatch[1]}.${fracMatch ? fracMatch[1] : '00'}`);
      if (price > 50 && price < product.basePrice * 2.5) {
        console.log(`[Amazon HTML] ✓ ${product.name}: $${price}`);
        return price;
      }
    }

    // Method 3: corePriceDisplay_desktop_feature_div
    const coreMatch = body.match(/corePriceDisplay[^"]*"[^>]*>[\s\S]{0,200}?\$([\d,]+\.?\d*)/);
    if (coreMatch) {
      const price = parseFloat(coreMatch[1].replace(',', ''));
      if (price > 50 && price < product.basePrice * 2.5) {
        console.log(`[Amazon Core] ✓ ${product.name}: $${price}`);
        return price;
      }
    }
  } catch (err) {
    console.warn(`[Amazon] Error for ${product.name}:`, err.message);
  }
  return null;
}

// ── B&H PHOTO SCRAPER ─────────────────────────────────────
async function fetchBH(product) {
  if (!product.bh_url) return null;
  try {
    const { status, body } = await fetchPage(product.bh_url, '&country_code=us');
    if (status !== 200 || !body) return null;

    const priceMatch = body.match(/"price"\s*:\s*"?([\d]+\.?\d*)"?/);
    if (priceMatch) {
      const price = parseFloat(priceMatch[1]);
      if (price > 50 && price < product.basePrice * 2) {
        console.log(`[B&H] ✓ ${product.name}: $${price}`);
        return price;
      }
    }
  } catch (err) {
    console.warn(`[B&H] Error for ${product.name}:`, err.message);
  }
  return null;
}
...
// ── FETCH A SINGLE PRODUCT PRICE (Multi-Source) ──────────
async function fetchPrice(product, currentPrice) {
  console.log(`\n[Fetch] ${product.name}`);

  // جمع الأسعار من جميع المصادر
  const prices = [];

  // 1. Newegg — الأسرع والأدق لقطع الكمبيوتر
  const neweggPrice = await fetchNewegg(product);
  if (neweggPrice) prices.push({ source: 'Newegg', price: neweggPrice });

  // 2. Amazon — إذا توفّر ASIN
  if (product.asin) {
    const amazonPrice = await fetchAmazon(product);
    if (amazonPrice) prices.push({ source: 'Amazon', price: amazonPrice });
  }

  // 3. BestBuy — إذا توفّر SKU
  if (product.bestbuy_sku) {
    const bbPrice = await fetchBestBuy(product);
    if (bbPrice) prices.push({ source: 'BestBuy', price: bbPrice });
  }

  // 4. B&H Photo — إذا توفّر رابط
  if (product.bh_url) {
    const bhPrice = await fetchBH(product);
    if (bhPrice) prices.push({ source: 'B&H', price: bhPrice });
  }

  if (prices.length > 0) {
    // اختر أدنى سعر من مصادر موثوقة
    prices.sort((a, b) => a.price - b.price);
    const best = prices[0];
    const avg  = Math.round(prices.reduce((s, p) => s + p.price, 0) / prices.length);
    console.log(`[Best] ${product.name}: $${best.price} via ${best.source} | avg: $${avg} | sources: ${prices.map(p => p.source).join(', ')}`);
    return best.price;
  }
...
async function fetchAllPrices(currentPrices = {}) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    console.log(`\n[${i+1}/${total}] Processing: ${product.name}`);
    results[product.id] = await fetchPrice(product, currentPrice);

    // تأخير 1.5 ثانية بين المنتجات لتجنب Rate Limiting في ScraperAPI
    if (i < total - 1) {
  ...
  console.log(`\n✓ Done: ${fetched}/${total} products priced`);
  return results;
}
